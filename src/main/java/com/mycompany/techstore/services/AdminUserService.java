...
public class AdminUserService {
    ...
    public void setStatus(int id,String status,int adminId)throws SQLException{
        if(id==adminId)throw new BackOfficeValidationException("You cannot change your own status.");
        if(!Set.of("Active","Blocked").contains(status))throw new BackOfficeValidationException("Invalid user status.");
        if("Blocked".equals(status)&&repository.isLastActiveAdmin(id))throw new BackOfficeValidationException("The last active administrator cannot be blocked.");
    ...
    public void setRole(int id,int roleId,int adminId)throws SQLException{
        ...
        AdminUser selected=repository.findById(id);if(selected==null)throw new BackOfficeValidationException("User not found.");
        if("Admin".equals(selected.getRoleName())&&repository.isLastActiveAdmin(id)){
            for(LookupOption role:repository.roles())if(role.getId()==roleId&&!"Admin".equals(role.getName()))throw new BackOfficeValidationException("The last active administrator cannot be demoted.");
        }
    ...
    public void setStatusForActor(int id,String status,int actorId,String actorRole)throws SQLException{
        ...
        if("Staff".equals(actorRole)&&!"Customer".equals(selected.getRoleName()))throw new BackOfficeValidationException("Staff can only block or unblock customer accounts.");
    ...
    public void createStaff(String name,String email,String phone,String password,int adminId)throws SQLException,NoSuchAlgorithmException{
        validateStaff(name,email,phone);if(password==null||password.length()<8)throw new BackOfficeValidationException("Password must contain at least 8 characters.");
        if(repository.emailExists(email,0))throw new BackOfficeValidationException("Email is already in use.");
    ...
    public void updateStaff(int id,String name,String email,String phone,int adminId)throws SQLException{
        ...
        if(current==null||!"Staff".equals(current.getRoleName()))throw new BackOfficeValidationException("Staff account not found.");
        if(repository.emailExists(email,id))throw new BackOfficeValidationException("Email is already in use.");
    ...
    public void deleteStaff(int id,int adminId)throws SQLException{
        if(id==adminId)throw new BackOfficeValidationException("You cannot delete your own account.");repository.deactivateStaff(id,adminId);
    }
    private void validateStaff(String name,String email,String phone){
        if(name==null||name.trim().length()<2)throw new BackOfficeValidationException("Staff name is required.");
        if(email==null||!email.trim().matches("^[^@\\s]+@[^@\\s]+\\.[^@\\s]+$"))throw new BackOfficeValidationException("A valid email is required.");
        if(phone!=null&&!phone.isBlank()&&!phone.trim().matches("[0-9+ .()-]{8,20}"))throw new BackOfficeValidationException("Invalid phone number.");
    }
