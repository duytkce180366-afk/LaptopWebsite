...
public class AdminReviewService {
    ...
    public PageResult<AdminReview> findAll(String q,int rating,String status,int page)throws SQLException{return repository.findAll(q,rating,status,Math.max(1,page),12);}
    public void setStatus(int id,String status,int adminId)throws SQLException{
        if(!Set.of("Visible","Hidden").contains(status))throw new BackOfficeValidationException("Invalid review status.");
