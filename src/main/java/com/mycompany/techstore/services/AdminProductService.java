...
public class AdminProductService {
    ...
    public void update(AdminProduct product,int adminId)throws SQLException{
        AdminProduct current=repository.findById(product.getProductId());if(current==null)throw new BackOfficeValidationException("Product not found.");
    ...
    public void receiveStock(int id,int quantity,String note,int adminId)throws SQLException{
        ...
        if(quantity<=0)throw new BackOfficeValidationException("Received quantity must be greater than zero.");
        if(quantity>100000)throw new BackOfficeValidationException("Received quantity is too large.");
    ...
    private void validate(AdminProduct p)throws SQLException{
        ...
        if(p.getProductName().isEmpty())throw new BackOfficeValidationException("Product name is required.");
        if(p.getCategoryId()<=0||p.getBrandId()<=0)throw new BackOfficeValidationException("Category and brand are required.");
        if(p.getPrice()==null||p.getPrice().compareTo(BigDecimal.ZERO)<=0)throw new BackOfficeValidationException("Price must be greater than zero.");
        if(p.getStock()<0)throw new BackOfficeValidationException("Stock cannot be negative.");
        if(repository.skuExists(p.getSku(),p.getProductId()))throw new BackOfficeValidationException("SKU already exists.");
        ...
        if(isLaptop(p.getCategoryId()))for(String key:LAPTOP_SPECS){
            if(clean(p.getSpecifications().get(key)).isEmpty())throw new BackOfficeValidationException("Laptop specification '"+key+"' is required.");
        }
    }
