...
public class AdminReportController extends HttpServlet {
    ...
    @Override protected void doGet(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
        ...
        try{
            ...
            req.setAttribute("rows",rows);req.setAttribute("type",type);req.setAttribute("from",from);req.setAttribute("to",to);
            req.getRequestDispatcher("/WEB-INF/JSPViews/AdminView/reports.jsp").forward(req,res);
        }catch(SQLException ex){throw new ServletException(ex);}
    }
    private void csv(HttpServletResponse res,String type,List<Map<String,Object>> rows)throws IOException{
        res.setContentType("text/csv;charset=UTF-8");res.setHeader("Content-Disposition","attachment; filename=techstore-"+type+"-report.csv");
        if(rows.isEmpty()){res.getWriter().println("No data");return;}
        List<String> headers=new ArrayList<>(rows.get(0).keySet());res.getWriter().println(String.join(",",headers));
        for(Map<String,Object> row:rows){List<String> cells=new ArrayList<>();for(String h:headers)cells.add(escape(row.get(h)));res.getWriter().println(String.join(",",cells));}
    }
    private String escape(Object value){String v=value==null?"":String.valueOf(value);return "\""+v.replace("\"","\"\"")+"\"";}
