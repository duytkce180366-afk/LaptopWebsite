...
public class AdminInventoryController extends HttpServlet {
    ...
    @Override protected void doGet(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
        try{
            String q=text(req,"q");int page=number(req,"page",1);
            req.setAttribute("result",service.findAll(q,0,0,"",page));req.setAttribute("receipts",service.recentReceipts());req.setAttribute("q",q);
    ...
    @Override protected void doPost(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
        ...
        try{service.receiveStock(number(req,"productId",0),number(req,"quantity",0),text(req,"note"),admin.getUser_id());req.getSession().setAttribute("adminMessage","Stock receipt was recorded.");}
