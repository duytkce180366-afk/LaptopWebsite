...
public class AuthController extends HttpServlet {
  ...
  public AuthController() {
    this.RootUrl =
        (System.getenv("ROOT_ENV") != null) ? System.getenv("ROOT_ENV") : "http://localhost:8080";
  ...
  private boolean LoadOidcConfiguration() {
    try {
      ...
      try (JsonReader reader = Json.createReader(conn.getInputStream())) {
        ...
        Logger.getLogger(AuthController.class.getName())
            .log(Level.INFO, "OIDC configuration loaded successfully.");
        return true;
      }
  ...
  private void HandleOidcCallback(HttpServletRequest request, HttpServletResponse response)
    ...
    if (status != 200) {
      response.sendRedirect(
          request.getContextPath() + "/auth?action=signin&error=Token+endpoint+returned+" + status);
      return;
    }
    ...
    try (JsonReader jr = Json.createReader(conn.getInputStream())) {
      ...
      if (idToken == null) {
        response.sendRedirect(
            request.getContextPath() + "/auth?action=signin&error=No+id_token+in+token+response");
        return;
      }
      ...
      try {
        ...
        JWKSource<SecurityContext> keySource =
            JWKSourceBuilder.create(jwksUrl, resourceRetriever).build();
        ...
        if (!claims.getAudience().contains(this.OidcClientId)) {
          response.sendRedirect(
              request.getContextPath() + "/auth?action=signin&error=Token+audience+mismatch.");
          return;
        }
        ...
        if (exp == null || exp.toInstant().isBefore(Instant.now())) {
          response.sendRedirect(
              request.getContextPath() + "/auth?action=signin&error=Token+expired.");
          return;
        }
  ...
  private void HandleResetPassword(HttpServletRequest request, HttpServletResponse response)
    ...
    if (newPwd == null || repeatPwd == null || !newPwd.equals(repeatPwd)) {
      response.sendRedirect(
          request.getContextPath() + "/home?error=Error+input.+Please+validate+the+password.");
      return;
    }
    ...
    try {
      ...
      if (refreshAction) {
        ...
        User userRefresh =
            this.authService.GetOrCreateUserOIDCSignIn(logged.getEmail(), logged.getFull_name());
  ...
  private void SetOTP(HttpServletRequest request) throws MessagingException, AuthException {
    ...
    if (lastReq != null && curr.isBefore(lastReq.plusSeconds(this.otpRelax))) {
      throw new AuthException(
          -1, "Requesting OTP too frequently. Please wait %s seconds.".formatted(this.otpRelax));
    }
  ...
  private boolean VerifyOTP(HttpServletRequest request, String expectedOtp) throws AuthException {
    ...
    return MessageDigest.isEqual(
        otp.getBytes(StandardCharsets.UTF_8), expectedOtp.getBytes(StandardCharsets.UTF_8));
  }
    ...
    switch (request.getParameter("action")) {
      ...
      case "signin" -> {
        if (this.IsSignedIn(request)) {
          ...
          request
              .getRequestDispatcher("/WEB-INF/JSPViews/AuthView/Login.jsp")
          ...
          try {
            ...
            if (verifyStatus) {
              ...
              Logger.getLogger(AuthController.class.getName())
                  .log(
                      Level.WARNING,
                      "SMTP_HOST not set and auto-verification failed for {0}",
                      logged.getEmail());
            }
          } catch (AuthException ex) {
            Logger.getLogger(AuthController.class.getName()).log(Level.SEVERE, null, ex);
            response.sendRedirect(
                request.getContextPath()
                    + "/home?error=Internal+server+error+during+auto-verification.");
            return;
          }
        }
        // Attempt to send OTP email only if EmailService is available; log failures but continue to
        // verification page
        if (this.emailService != null) {
          try {
            this.SetOTP(request);
            ...
            Logger.getLogger(AuthController.class.getName())
                .log(Level.WARNING, "Failed to send OTP email: " + mex.getMessage(), mex);
          } catch (AuthException ex) {
            Logger.getLogger(AuthController.class.getName())
                .log(Level.SEVERE, ex.getLocalizedMessage(), ex);
            String errorEx = URLEncoder.encode(ex.getMessage(), StandardCharsets.UTF_8);
          ...
          Logger.getLogger(AuthController.class.getName())
              .log(Level.INFO, "EmailService disabled; not sending OTP email.");
        }

        request
            .getRequestDispatcher("/WEB-INF/JSPViews/AuthView/VerifyOTP.jsp")
            .forward(request, response);
      }
      // OIDC
      case "oidc_signin" -> {
        if (this.IsSignedIn(request)) {
          ...
          if (this.isOidcEnabled) {
            ...
            response.sendRedirect(
                request.getContextPath() + "/auth?action=signin&error=OIDC+is+not+enabled.");
          }
        }
      }
      case "oidc_callback" -> {
      ...
      case "resetpwd" -> {
        if (this.IsSignedIn(request)) {
          ...
          request
              .getRequestDispatcher("/WEB-INF/JSPViews/AuthView/ResetPwd.jsp")
              .forward(request, response);
        } else {
          response.sendRedirect(
              request.getContextPath() + "/auth?action=signin&error=Not+signed+in.");
        }
      }
    ...
    switch (request.getParameter("action")) {
      case "signin" -> {
        if (!this.IsSignedIn(request)) {
          ...
          response.sendRedirect(
              request.getContextPath() + "/auth?action=signin&error=Already+signed+in.");
        }
      }
      case "signup" -> {
        if (!this.IsSignedIn(request)) {
      ...
      default -> {
        ...
        request
            .getRequestDispatcher("/WEB-INF/JSPViews/AuthView/Denied.jsp")
            .forward(request, response);
      }
    }
  }
