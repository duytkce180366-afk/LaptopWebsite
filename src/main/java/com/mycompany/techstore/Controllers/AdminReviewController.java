...
public class AdminReviewController extends HttpServlet {
    ...
    @Override protected void doGet(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
        try{
            String q=text(req,"q"),status=text(req,"status");int rating=number(req,"rating",0),page=number(req,"page",1);
            req.setAttribute("result",service.findAll(q,rating,status,page));req.setAttribute("q",q);req.setAttribute("selectedRating",rating);req.setAttribute("selectedStatus",status);
    ...
    @Override protected void doPost(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
        ...
        try{service.setStatus(number(req,"id",0),text(req,"status"),admin.getUser_id());req.getSession().setAttribute("adminMessage","Review visibility was updated.");}
