...
public class AdminProductController extends HttpServlet {
    ...
    @Override protected void doGet(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
        try{
            ...
            if("/new".equals(path)){showForm(req,res,new AdminProduct());return;}
            if("/edit".equals(path)){AdminProduct p=service.findById(intParam(req,"id",0));if(p==null){res.sendError(404);return;}showForm(req,res,p);return;}
            list(req,res);
    ...
    @Override protected void doPost(HttpServletRequest req,HttpServletResponse res)throws ServletException,IOException{
        User admin=(User)req.getSession().getAttribute("loggedUser");String path=req.getPathInfo();
        try{
            if("/deactivate".equals(path)){
                service.deactivate(intParam(req,"id",0),admin.getUser_id());flash(req,"Product was deactivated.");
            }else{
                AdminProduct product=fromRequest(req);
                if(product.getProductId()==0){service.create(product,admin.getUser_id());flash(req,"Product was created.");}
                else{service.update(product,admin.getUser_id());flash(req,"Product was updated.");}
            }
            res.sendRedirect(req.getContextPath()+"/admin/products");
        }catch(IllegalArgumentException ex){
            req.setAttribute("error",ex.getMessage());try{showForm(req,res,fromRequest(req));}catch(SQLException sql){throw new ServletException(sql);}
        }catch(SQLException ex){throw new ServletException(ex);}
    }
    private void list(HttpServletRequest req,HttpServletResponse res)throws SQLException,ServletException,IOException{
        String q=value(req,"q"),status=value(req,"status");int category=intParam(req,"category",0),brand=intParam(req,"brand",0),page=intParam(req,"page",1);
        req.setAttribute("result",service.findAll(q,category,brand,status,page));
        req.setAttribute("categories",service.categories());req.setAttribute("brands",service.brands());
        req.setAttribute("q",q);req.setAttribute("selectedCategory",category);req.setAttribute("selectedBrand",brand);req.setAttribute("selectedStatus",status);
    ...
    private void showForm(HttpServletRequest req,HttpServletResponse res,AdminProduct p)throws SQLException,ServletException,IOException{
        req.setAttribute("product",p);req.setAttribute("categories",service.categories());req.setAttribute("brands",service.brands());
    ...
    private AdminProduct fromRequest(HttpServletRequest req){
        ...
        p.setSpecifications(specs);return p;
    }
    private int intParam(HttpServletRequest r,String n,int d){try{return Integer.parseInt(value(r,n));}catch(Exception ex){return d;}}
    private BigDecimal decimal(HttpServletRequest r,String n){try{return new BigDecimal(value(r,n));}catch(Exception ex){return null;}}
    private String value(HttpServletRequest r,String n){String v=r.getParameter(n);return v==null?"":v.trim();}
    private void flash(HttpServletRequest req,String message){req.getSession().setAttribute("adminMessage",message);}
}
