...
public class AccountStatusFilter implements Filter {
  ...
  private static final Set<String> STATIC_PREFIXES =
      Set.of("/css/", "/js/", "/lib/", "/images/", "/img/", "/fonts/", "/favicon");
  ...
  public void doFilter(ServletRequest rawRequest, ServletResponse rawResponse, FilterChain chain)
    ...
    try {
      ...
      if (currentUser == null || !"Active".equalsIgnoreCase(currentUser.getStatus())) {
        String reason =
            currentUser != null && "Blocked".equalsIgnoreCase(currentUser.getStatus())
                ? "blocked"
                : "disabled";
        session.invalidate();
      ...
      request.setAttribute(
          "isBackOfficeAccount", BackOfficeAuthorizationPolicy.isBackOfficeRole(currentRole));
      chain.doFilter(request, response);
