...
public class UserRepository {
  ...
  public String findRoleName(int userId) throws SQLException {
    String sql =
        "SELECT r.role_name FROM dbo.bs_user u JOIN dbo.bs_Roles r ON r.role_id=u.role_id WHERE"
            + " u.user_id=?";
    DbClass database = new DbClass();
    Connection connection = database.getConnection();
    if (connection == null) throw new SQLException("Database connection is unavailable.");
    try (connection;
