...
public class AdminProductRepository {
    ...
    public int create(AdminProduct p,int adminId)throws SQLException{
        ...
        try(Connection con=new DbClass().getConnection()){con.setAutoCommit(false);try(PreparedStatement ps=con.prepareStatement(sql,Statement.RETURN_GENERATED_KEYS)){
            bindCreate(ps,p);ps.executeUpdate();try(ResultSet keys=ps.getGeneratedKeys()){if(!keys.next())throw new SQLException("Product ID was not generated");p.setProductId(keys.getInt(1));}
    ...
    public void update(AdminProduct p,int adminId)throws SQLException{
        String sql="UPDATE dbo.bs_Products SET category_id=?,brand_id=?,sku=?,product_name=?,description=?,price=?,thumbnail=?,status=?,updated_at=SYSUTCDATETIME() WHERE product_id=?";
        try(Connection con=new DbClass().getConnection()){con.setAutoCommit(false);try(PreparedStatement ps=con.prepareStatement(sql)){
            bindUpdate(ps,p);ps.setInt(9,p.getProductId());if(ps.executeUpdate()!=1)throw new SQLException("Product not found");
    ...
    public void receiveStock(int productId,int quantity,String note,int adminId)throws SQLException{
        try(Connection con=new DbClass().getConnection()){con.setAutoCommit(false);try{
            int previous;
            try(PreparedStatement lock=con.prepareStatement("SELECT stock FROM dbo.bs_Products WITH (UPDLOCK,ROWLOCK) WHERE product_id=?")){
                lock.setInt(1,productId);try(ResultSet rs=lock.executeQuery()){if(!rs.next())throw new SQLException("Product not found");previous=rs.getInt(1);}}
            int resulting=previous+quantity;
            try(PreparedStatement update=con.prepareStatement("UPDATE dbo.bs_Products SET stock=?,status=CASE WHEN status='Out of Stock' THEN 'Active' ELSE status END,updated_at=SYSUTCDATETIME() WHERE product_id=?")){
                update.setInt(1,resulting);update.setInt(2,productId);update.executeUpdate();}
            try(PreparedStatement receipt=con.prepareStatement("INSERT INTO dbo.bs_StockReceipts(product_id,quantity,previous_stock,resulting_stock,note,admin_id) VALUES(?,?,?,?,?,?)")){
                receipt.setInt(1,productId);receipt.setInt(2,quantity);receipt.setInt(3,previous);receipt.setInt(4,resulting);receipt.setString(5,note);receipt.setInt(6,adminId);receipt.executeUpdate();}
            audit.log(con,adminId,"STOCK_RECEIPT","PRODUCT",productId,"+"+quantity+" ("+previous+" -> "+resulting+")"+(note.isBlank()?"":"; "+note));
            con.commit();
        }catch(Exception ex){con.rollback();throw ex;}}
    }
    ...
    private void bindUpdate(PreparedStatement ps,AdminProduct p)throws SQLException{
        ps.setInt(1,p.getCategoryId());ps.setInt(2,p.getBrandId());ps.setString(3,p.getSku());ps.setString(4,p.getProductName());
        ps.setString(5,p.getDescription());ps.setBigDecimal(6,p.getPrice());ps.setString(7,p.getThumbnail());ps.setString(8,p.getStatus());
    }
