...
public class AdminReviewRepository {
    ...
    public PageResult<AdminReview> findAll(String q,int rating,String status,int page,int size)throws SQLException{
        String sql="""
        ...
        List<AdminReview> items=new ArrayList<>();int total=0;String search=clean(q),like="%"+search+"%",s=clean(status);
        try(Connection con=new DbClass().getConnection();PreparedStatement ps=con.prepareStatement(sql)){
            ps.setString(1,search);ps.setString(2,like);ps.setString(3,like);ps.setString(4,like);ps.setString(5,like);
            ps.setInt(6,rating);ps.setInt(7,rating);ps.setString(8,s);ps.setString(9,s);ps.setInt(10,(page-1)*size);ps.setInt(11,size);
    ...
    public void setStatus(int id,String status,int adminId)throws SQLException{
        try(Connection con=new DbClass().getConnection()){con.setAutoCommit(false);try(PreparedStatement ps=con.prepareStatement(
                "UPDATE dbo.bs_Reviews SET status=?,moderated_by=?,moderated_at=SYSUTCDATETIME(),updated_at=SYSUTCDATETIME() WHERE review_id=?")){
            ps.setString(1,status);ps.setInt(2,adminId);ps.setInt(3,id);if(ps.executeUpdate()!=1)throw new SQLException("Review not found");
            audit.log(con,adminId,"MODERATE","REVIEW",id,status);con.commit();
